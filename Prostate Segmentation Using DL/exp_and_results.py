...
def dice_metric(y_true, y_pred, axis=None, smooth=1.0):
...
# To compute the haussdorff distance here is uing the following function, taken from https://github.com/mirzaevinom/promise12_segmentation/blob/master/codes/metrics.py
def surface_dist(input1, input2, sampling=1, connectivity=1):
    ...
    conn = ndimage.generate_binary_structure(input_1.ndim, connectivity)
    # Generate a binary structure for binary morphological operations.

    # S = (input_1.astype(np.float32) - ndimage.binary_erosion(input_1, conn).astype(np.float32)).astype(np.bool_)
    # Sprime = (input_2.astype(np.float32) - ndimage.binary_erosion(input_2, conn).astype(np.float32)).astype(np.bool_)
    S = input_1 ^ ndimage.binary_erosion(input_1, conn)
    Sprime = input_2 ^ ndimage.binary_erosion(input_2, conn)
    dta = ndimage.distance_transform_edt(~S, sampling)  # distance to a
    dtb = ndimage.distance_transform_edt(~Sprime, sampling)  # distance to b
...
# inspired by : https://github.com/mirzaevinom/promise12_segmentation/blob/master/codes/metrics.py 
def check_predictions(the_list, data_set="test1", y_pred=[]):
    if not os.path.isdir('E:/Users/Atithi/Downloads/Project/venv/Promise12/TestData/predictions/'):
        os.mkdir('E:/Users/Atithi/Downloads/Project/venv/Promise12/TestData/predictions/')

    if data_set == "train":
        X = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/X_train.npy')
        y = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/y_train.npy')
    elif data_set == "test":
        X = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/X_test.npy')
        y = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/y_test.npy')
    # elif data_set == "test1":
    # X = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TestData/X_test1.npy')
    # y = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TestData/y_test1.npy')
    else:
        X = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/X_val.npy')
        y = np.load('E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/y_val.npy')
    # ----------------------------------------------------------------------------------------------
    # X_test1,y_test1= load_only_testing()
    # X_train, y_train, X_val, y_val, X_test, y_test = load_data()

    # model = get_model(256, 256, 1, 'E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/weights_no_augmentation.h5')
    ...
    print('Mean volumetric DSE:', vol_scores.mean())
    print('Median volumetric DSE:', np.median(vol_scores))
    print('Std volumetric DSE:', vol_scores.std())
...
# inspired by : https://github.com/mirzaevinom/promise12_segmentation/blob/master/codes/metrics.py
def read_cases(the_list=None, folder='E:/Users/Atithi/Downloads/Project/venv/Promise12/TrainData/', masks=True):
...
def get_model(img_rows, img_cols, img_depth, path):
    model = UNet((img_rows, img_cols, img_depth), start_ch=8, depth=5, batchnorm=True, dropout=0.5, maxpool=True,
                 residual=True)
...
def test_with_augmentation():
    ...
    model = get_model(256, 256, 1,
                      'E:/Users/Atithi/Downloads/Project/venv/Application-of-Deep-Learning-to-Prostate-Cancer-master/Application-of-Deep-Learning-to-Prostate-Cancer-master/Promise12_Final/data/weights_with_augmentation.h5')
    y_pred = model.predict(X_test, verbose=1, batch_size=128)
    check_predictions(the_list=[42, 43, 44, 45, 46, 47, 48, 49], data_set="test", y_pred=y_pred)
